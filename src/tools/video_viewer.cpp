...
{
    VideoViewer vv(input_uri,output_uri,false);
}


VideoViewer::VideoViewer(const std::string& input_uri, const std::string& output_uri, const bool new_thread)
    : video_(input_uri,output_uri),
      video_wait_("video.wait", true),
      video_newest_("video.newest", false),
      thread_(nullptr),
      toquit_(false)
{
    if(new_thread)
    {
        thread_.reset(new std::thread(&VideoViewer::RunUI,this));
    }
    else
        RunUI();
}
...
pangolin::Var<bool>& VideoViewer::VideoWait()
{
    return video_wait_;
}

pangolin::Var<bool>& VideoViewer::VideoNewest()
{
    return video_newest_;
}

void VideoViewer::StartRecording()
{
    std::lock_guard<std::mutex> guard(mutex);
    
    if(!video_.IsRecording()) {
...
{
    std::lock_guard<std::mutex> guard(mutex);
    video_.RecordOneFrame();
}


void VideoViewer::ChangeOutputURL(const std::string& url)
{
    std::lock_guard<std::mutex> guard(mutex);
    
    video_.LogFilename()=url;
}

void VideoViewer::RunUI()
{
    ...
    View& container = pangolin::Display("streams");
...

    {
        ...
        // Open Video by URI
        const size_t num_streams = video_.Streams().size();
        ...
        for(size_t s = 0; s < num_streams; ++s) {
            const pangolin::StreamInfo& si = video_.Streams()[s];
        ...
        video_playback = pangolin::FindFirstMatchingVideoInterface<pangolin::VideoPlaybackInterface>(video_);
        ...
        frame = Var<int>("ui.frame", -1, 0, total_frames-1 );
        ...
        buffer.resize(video_.SizeBytes()+1);

        // Create OpenGL window - guess sensible dimensions
        pangolin::CreateWindowAndBind( "VideoViewer",
                                       (int)(video_.Width() * num_streams),
                                       (int)(video_.Height() + slider_size)
                                       );
        ...
        for(unsigned int d=0; d < num_streams; ++d) {
            const pangolin::StreamInfo& si = video_.Streams()[d];
        ...
        for(size_t v=0; v < container.NumChildren() && v < 9; v++) {
            ...
            pangolin::RegisterKeyPressCallback(screenshot_keys[v], [v,&images,this](){
                if(v < images.size() && images[v].ptr) {
                    try{
                        pangolin::SaveImage(
                                    images[v], video_.Streams()[v].PixFormat(),
        ...
        pangolin::RegisterKeyPressCallback('r', [&](){
            if(!video_.IsRecording()) {
                video_.SetTimelapse( static_cast<size_t>(record_timelapse_frame_skip) );
                video_.Record();
                pango_print_info("Started Recording.\n");
            }else{
                video_.Stop();
    ...
    while(!pangolin::ShouldQuit() && !toquit_)
    {
        {
            std::lock_guard<std::mutex> guard(mutex);
...
#endif
            if ( frame < end_frame ) {
                if( video_.Grab(&buffer[0], images, video_wait_, video_newest_) ) {
                    frame = frame +1;
                }
            }
#ifdef DEBUGVIDEOVIEWER
    ...
    pangolin::DestroyWindow("VideoViewer");
}
