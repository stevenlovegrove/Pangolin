...
int main(int argc, char** argv)
{
  ...
  argagg::parser argparser{{
      ...
      {"model",
       {"-m", "--model", "--mesh"},
       "3D Model to load (obj or ply)",
       1},
      {"matcap",
       {"--matcap"},
       "Matcap (material capture) images to load for shading",
       1},
      {"envmap", {"--envmap", "-e"}, "Equirect environment map for skybox", 1},
      {"mode",
       {"--mode"},
       "Render mode to use {show_uv, show_texture, show_color, show_normal, "
      ...
      {"spin",
       {"--spin"},
       "Spin models around an axis {none, negx, x, negy, y, negz, z}",
       1},
  }};
  ...
  const std::string mode_names[] = {"SHOW_UV",     "SHOW_TEXTURE",
                                    "SHOW_COLOR",  "SHOW_NORMAL",
                                    "SHOW_MATCAP", "SHOW_VERTEX"};
  ...
  for (int i = 0; i <= (int)pangolin::AxisZ; ++i)
    if (pangolin::ToUpperCopy(args["spin"].as<std::string>("none")) ==
        spin_names[i])
  ...
  auto show_renderable = [&](int index) {
    ...
    for (size_t i = 0; i < renderables.size(); ++i) {
      if (renderables[i])
        renderables[i]->show = (index == -1 || index == (int)i) ? true : false;
    }
  };
    ...
    for (auto& future_geom : geom_to_load) {
      if (future_geom.valid() && is_ready(future_geom)) {
        ...
        const Eigen::Vector3f view =
            center + Eigen::Vector3f(1.2, 0.8, 1.2) *
                         std::max(
                             (total_aabb.max() - center).norm(),
                             (center - total_aabb.min()).norm());
        const auto mvm = pangolin::ModelViewLookAt(
            view[0], view[1], view[2], center[0], center[1], center[2],
            pangolin::AxisY);
        const double far = 100.0 * (total_aabb.max() - total_aabb.min()).norm();
        const double near = far / 1e6;
        const auto proj =
            pangolin::ProjectionMatrix(w, h, f, f, w / 2.0, h / 2.0, near, far);
  ...
  auto LoadProgram = [&](const RenderMode mode) {
    ...
    default_prog.AddShader(
        pangolin::GlSlAnnotatedShader, default_model_shader, prog_defines);
    default_prog.Link();
  };
  ...
  for (int i = 0; i < (int)RenderMode::num_modes; ++i)
    pangolin::RegisterKeyPressCallback(
        mode_key[i], [&, i]() { LoadProgram((RenderMode)i); });
  ...
  pangolin::RegisterKeyPressCallback(PANGO_SPECIAL + PANGO_KEY_LEFT, [&]() {
    if (renderables.size())
      show_renderable(
          (mesh_to_show + renderables.size() - 1) % renderables.size());
  });
  pangolin::RegisterKeyPressCallback(' ', [&]() { show_renderable(-1); });

  if (matcaps.size()) {
    pangolin::RegisterKeyPressCallback(
        '=', [&]() { matcap_index = (matcap_index + 1) % matcaps.size(); });
    pangolin::RegisterKeyPressCallback('-', [&]() {
  ...
  if (envmaps.size()) {
    pangolin::RegisterKeyPressCallback(
        ']', [&]() { envmap_index = ((envmap_index + 1) % envmaps.size()); });
    pangolin::RegisterKeyPressCallback('[', [&]() {
      envmap_index = ((envmap_index + envmaps.size() - 1) % envmaps.size());
    });
  }
  pangolin::RegisterKeyPressCallback(
      's', [&]() { std::swap(spin_transform->dir, spin_other); });
  pangolin::RegisterKeyPressCallback(
      'b', [&]() { show_bounds = !show_bounds; });
  pangolin::RegisterKeyPressCallback(
      '0', [&]() { cull_backfaces = !cull_backfaces; });
  ...
  while (!pangolin::ShouldQuit()) {
    if ((handler.Selected_P_w() - Pick_w).norm() > 1E-6) {
      ...
      std::cout << pangolin::FormatString(
                       "\"Translation\": [%,%,%]", Pick_w[0], Pick_w[1],
                       Pick_w[2])
                << std::endl;
    }
    ...
    if (d_cam.IsShown()) {
      ...
      if (env_prog.Valid()) {
        ...
        const Eigen::Matrix3f R_env_cam =
            mvmat.block<3, 3>(0, 0).cast<float>().transpose();
        Eigen::Matrix3f Kinv;
        Kinv << 1.0 / f, 0.0, -(w / 2.0) / f, 0.0, 1.0 / f, -(h / 2.0) / f, 0.0,
            0.0, 1.0;
        ...
        if (vertex_handle >= 0 && xy_handle >= 0) {
          ...
          const GLfloat ndc[] = {1.0f,  1.0f,  -1.0f, 1.0f,
                                 -1.0f, -1.0f, 1.0f,  -1.0f};
          const GLfloat pix[] = {0.0f, 0.0f, w, 0.0f, w, h, 0.0f, h};

          glEnableVertexAttribArray(vertex_handle);
          glVertexAttribPointer(
              vertex_handle, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), ndc);
          glEnableVertexAttribArray(xy_handle);
          glVertexAttribPointer(
              xy_handle, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), pix);
      ...
      default_prog.Bind();
      render_tree(
          default_prog, root, s_cam.GetProjectionMatrix(),
