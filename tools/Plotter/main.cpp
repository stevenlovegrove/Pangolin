...
int main(int argc, char* argv[])
{
  ...
  argagg::parser argparser{{
      ...
      {"x",
       {"-x"},
       "X-axis series to plot, seperated by commas (default: '$i')",
       1},
      {"y",
       {"-y"},
       "Y-axis series to plot, seperated by commas (eg: "
       "'$0,sin($1),sqrt($2+$3)' )",
       1},
      {"delim", {"-d"}, "Expected column delimitter (default: ',')", 1},
      {"xrange",
       {"-X", "--x-range"},
       "X-Axis min:max view (default: '0:100')",
       1},
      {"yrange",
       {"-Y", "--y-range"},
       "Y-Axis min:max view (default: '0:100')",
       1},
      {"skip",
       {"-s", "--skip"},
       "Skip n rows of file, seperated by commas per file (default: '0,...')",
       1},
  }};
  ...
  if ((bool)args["help"] || !args.pos.size()) {
    std::cerr << "Usage: Plotter [options] file1.csv [fileN.csv]*" << std::endl
              << argparser << std::endl
              << "    where: $i is a placeholder for the datum index,"
              << std::endl
              << "           $0, $1, ... are placeholders for the 0th, 1st, "
                 "... sequential datum values over the input files"
              << std::endl;
    return 0;
  }
  ...
  const pangolin::Rangef yrange =
      args["yrange"].as<>(pangolin::Rangef(0.0f, 100.0f));
  ...
  if (!(skipvec.size() == 0 || skipvec.size() == args.count())) {
    std::cerr << "Skip argument must be empty or correspond to the number of "
                 "files specified ("
              << args.count() << ")" << std::endl;
    return -1;
  }
  ...
  // Load asynchronously incase the file is large or is being read interactively
  // from stdin
  ...
  std::thread data_thread([&]() {
    ...
    while (keep_loading && csv_loader.ReadRow(row)) {
      std::vector<float> row_num(
          row.size(), std::numeric_limits<float>::quiet_NaN());
      for (size_t i = 0; i < row_num.size(); ++i) {
        try {
          row_num[i] = std::stof(row[i]);
        } catch (const std::invalid_argument&) {
          std::cerr << "Warning: couldn't parse '" << row[i]
                    << "' as numeric data (use -H option to include header)"
                    << std::endl;
        }
      }
      log.Log(row_num);
  ...
  pangolin::Plotter plotter(
      &log, xrange.min, xrange.max, yrange.min, yrange.max, 0.001, 0.001);
  if ((bool)args["x"] || (bool)args["y"]) {
    ...
    if (!(xvec.size() == 1 || xvec.size() == yvec.size())) {
      std::cout
          << "x-series dimensions must be one, or equal to y-series dimensions"
          << std::endl;
      return -1;
    }
