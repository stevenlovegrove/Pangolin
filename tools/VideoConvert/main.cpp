...
void VideoConvert(const std::string& input_uri, const std::string& output_uri)
{
  ...
  pangolin::VideoPlaybackInterface* playback =
      pangolin::FindFirstMatchingVideoInterface<
  ...
  while (true) {
    ...
    if (playback) {
      std::cout << "Frames complete: " << playback->GetCurrentFrameId() << " / "
                << playback->GetTotalFrames() << '\r';
      std::cout.flush();
    }
  }
}
...
int main(int argc, char* argv[])
{
  argagg::parser argparser = {
      {{"help", {"-h", "--help"}, "shows this help! duh!", 0},
       ...
       {"verbose",
        {"-v", "--verbose"},
        "verbose level in number, 0=list of schemes(default),1=scheme "
        "parameters,2=parameter details",
        1}}};
  ...
  if (args["help"] || args.pos.size() == 0) {
    ...
    std::cerr << "Examples:\n";
    std::cerr << "  VideoConvert test:[size=160x120,n=1,fmt=RGB24]//   Show "
                 "the 'test' video driver with 160x120 resolution, 1 stream, "
                 "RGB format.\n";
    std::cerr << "  VideoConvert --help -s image                       Find "
                 "out how to use the 'image' video driver\n\n";
    std::cerr << "Options:\n";
