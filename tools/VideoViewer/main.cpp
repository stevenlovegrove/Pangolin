...
int main(int argc, char* argv[])
{
  ...
  if (args["help"] || args.pos.size() == 0) {
    ...
    std::cerr << "Examples:\n";
    std::cerr << "  VideoViewer test:[size=160x120,n=1,fmt=RGB24]//   Show the "
                 "'test' video driver with 160x120 resolution, 1 stream, RGB "
                 "format.\n";
    std::cerr << "  VideoViewer --help -s image                       Find out "
                 "how to use the 'image' video driver\n\n";
    std::cerr << "Options:\n";
