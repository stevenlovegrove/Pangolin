...
struct SceneHandler : public Handler3D {
  SceneHandler(Renderable& scene, OpenGlRenderState& cam_state) :
      Handler3D(cam_state), scene(scene)
  {
  }

  void ProcessHitBuffer(
      GLint hits, GLuint* buf, std::map<GLuint, SelectedObject>& hit_map)
  {
  ...
  void Mouse(
      pangolin::View& view, pangolin::MouseButton button, int x, int y,
      bool pressed, int button_state)
  {
    ...
    for (auto selected_object_it = m_selected_objects.begin();
         ...
         selected_object_it++) {
      Interactive* ir =
          dynamic_cast<Interactive*>(selected_object_it->second.interactive);
      handled |= ir && ir->Mouse(
                           button, p, Pw, n, pressed, button_state,
                           selected_object_it->second.pickId);
    }
  ...
  void MouseMotion(pangolin::View& view, int x, int y, int button_state)
  {
    ...
    for (auto selected_object_it = m_selected_objects.begin();
      ...
      handled |=
          ir && ir->MouseMotion(
                    p, Pw, n, button_state, selected_object_it->second.pickId);
    }
    if (!handled) {
  ...
  void Special(
      pangolin::View& view, pangolin::InputSpecial inType, float x, float y,
      float p1, float p2, float p3, float p4, int button_state)
  {
    ...
    if (inType == pangolin::InputSpecialScroll) {
      m_selected_objects.clear();
      ComputeHits(
          view, *cam_state, (int)x, (int)y, 2 * hwin + 1, m_selected_objects);
      ...
      for (auto selected_object_it = m_selected_objects.begin();
        ...
        Interactive* ir =
            dynamic_cast<Interactive*>(selected_object_it->second.interactive);
        handled |= ir && ir->Mouse(
                             button, p, Pw, n, true, button_state,
                             selected_object_it->second.pickId);
      }
    }
    ...
    if (!handled) {
      pangolin::Handler3D::Special(
          view, inType, x, y, p1, p2, p3, p4, button_state);
    }
  }
