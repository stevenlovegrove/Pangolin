...
class PANGOLIN_EXPORT ImageView : public pangolin::View,
                                  public pangolin::ImageViewHandler
{
  ...
  void Keyboard(
      View& view, unsigned char key, int x, int y, bool pressed) override;

  pangolin::GlTexture& Tex();

  ImageView& SetImage(
      void* ptr, size_t w, size_t h, size_t pitch,
      ...
      const pangolin::Image<T>& img, bool delayed_upload = false)
  {
    return SetImage(
        ...
        GlPixFormat::FromType<T>(), delayed_upload);
  }
