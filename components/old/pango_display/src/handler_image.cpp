...
void ImageViewHandler::glSetViewOrtho()
{
  ...
  ProjectionMatrixOrthographic(
      xy.x.min, xy.x.max, xy.y.max, xy.y.min, -1.0f, 1.0f)
      .Load();
...
void ImageViewHandler::glRenderTexture(GLuint tex, GLint width, GLint height)
{
  if (tex != 0) {
    ...
    glTexParameteri(
        GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, UseNN() ? GL_NEAREST : GL_LINEAR);
    glTexParameteri(
        GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, UseNN() ? GL_NEAREST : GL_LINEAR);
...
void ImageViewHandler::glRenderTexture(
...
void ImageViewHandler::glRenderOverlay()
{
  ...
  if (std::abs(selxy.Area()) > 0) {
    ...
    font.Text("%.2f x %.2f", selxy.x.Size(), selxy.y.Size())
        .DrawWindow(xpix, ypix);

    font.Text(
            "(%.1f,%.1f)->(%.1f,%.1f)", selxy.x.min, selxy.y.min, selxy.x.max,
            selxy.y.max)
...
void ImageViewHandler::ScreenToImage(
...
void ImageViewHandler::ImageToScreen(
...
bool ImageViewHandler::UseNN() const { return use_nn; }

bool& ImageViewHandler::UseNN() { return use_nn; }

bool& ImageViewHandler::FlipTextureX() { return flipTextureX; }

bool& ImageViewHandler::FlipTextureY() { return flipTextureY; }

pangolin::XYRangef& ImageViewHandler::GetViewToRender() { return rview; }
...
pangolin::XYRangef& ImageViewHandler::GetView() { return target; }

pangolin::XYRangef& ImageViewHandler::GetDefaultView() { return rview_default; }

pangolin::XYRangef& ImageViewHandler::GetSelection() { return selection; }
...
void ImageViewHandler::Keyboard(
    View&, unsigned char key, int /*x*/, int /*y*/, bool pressed)
{
  ...
  if (pressed) {
    if (key == '\r') {
      ...
      pango_print_debug(
          "Unhandled ImageViewHandler::Keyboard. Key: %u\n", (unsigned int)key);
    }
  }
}

void ImageViewHandler::Mouse(
...
void ImageViewHandler::MouseMotion(View& view, int x, int y, int button_state)
{
  ...
  if (button_state == MouseButtonLeft) {
    ...
    Special(
        view, InputSpecialScroll, (float)x, (float)y, (float)d[0], (float)d[1],
        0.0f, 0.0f, button_state);
  }
...
void ImageViewHandler::PassiveMouseMotion(
    View&, int /*x*/, int /*y*/, int /*button_state*/)
{
}

void ImageViewHandler::Special(
...
void ImageViewHandler::AdjustScale()
{
  ...
  if (tv.target.x.AbsSize() > tv.rview_max.x.AbsSize())
    tv.target.x.Scale(
        tv.rview_max.x.AbsSize() / tv.target.x.AbsSize(), tv.target.x.Mid());
  if (tv.target.y.AbsSize() > tv.rview_max.y.AbsSize())
    tv.target.y.Scale(
        tv.rview_max.y.AbsSize() / tv.target.y.AbsSize(), tv.target.y.Mid());
}
...
void ImageViewHandler::AdjustTranslation()
{
  ...
  if (tv.target.y.max > tv.rview_max.y.max)
    tv.target.y -= tv.target.y.max - tv.rview_max.y.max;
  if (tv.target.y.min < tv.rview_max.y.min)
    tv.target.y -= tv.target.y.min - tv.rview_max.y.min;
}
