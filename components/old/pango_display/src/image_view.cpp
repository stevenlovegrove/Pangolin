...
void ImageView::Render()
{
  ...
  if (tex.IsValid()) {
    if (offset_scale.first != 0.0 || offset_scale.second != 1.0) {
      pangolin::GlSlUtilities::OffsetAndScale(
          offset_scale.first, offset_scale.second);
    } else {
      glColor4f(1, 1, 1, 1);
    }
...
void ImageView::Keyboard(
    View& view, unsigned char key, int x, int y, bool pressed)
{
  if (key == 'a') {
    if (!tex.IsValid()) {
      std::cerr << "ImageViewHandler does not contain valid texture."
                << std::endl;
      return;
    }
    ...
    tex.Download(img);
    offset_scale =
        pangolin::GetOffsetScale(img, pangolin::Round(froi), img.fmt);
  } else if (key == 'b') {
    if (!tex.IsValid()) {
      std::cerr << "ImageViewHandler does not contain valid texture."
                << std::endl;
    ...
    std::pair<float, float> mm =
        pangolin::GetMinMax(img, pangolin::Round(froi), img.fmt);
...
pangolin::GlTexture& ImageView::Tex() { return tex; }

ImageView& ImageView::SetImage(
    void* ptr, size_t w, size_t h, size_t pitch, pangolin::GlPixFormat img_fmt,
    bool delayed_upload)
{
  ...
  if (delayed_upload || !pangolin::GetBoundWindow() || IsDevicePtr(ptr) ||
    ...
    if (!convert_first) {
      ...
      PitchedCopy(
          (char*)img_to_load.ptr, img_to_load.pitch, (char*)ptr, pitch,
          w * pix_bytes, h);
  ...
  if (!tex.tid || tex.width != (int)w || tex.height != (int)h ||
    ...
    tex.Reinitialise(
        w, h, img_fmt.scalable_internal_format, true, 0, img_fmt.glformat,
        img_fmt.gltype, ptr);
  } else {
...
ImageView& ImageView::SetImage(const pangolin::GlTexture& texture)
{
  ...
  if (!tex.tid || tex.width != texture.width || tex.height != texture.height ||
    ...
    tex.Reinitialise(
        texture.width, texture.height, texture.internal_format, true);
  }

  glCopyImageSubData(
      texture.tid, GL_TEXTURE_2D, 0, 0, 0, 0, tex.tid, GL_TEXTURE_2D, 0, 0, 0,
      0, tex.width, tex.height, 1);
...
std::pair<float, float>& ImageView::GetOffsetScale() { return offset_scale; }

bool ImageView::MouseReleased() const { return mouseReleased; }
