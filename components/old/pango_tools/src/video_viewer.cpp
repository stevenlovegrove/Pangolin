...
{
  pango_print_info(
      "Caught signal. Program will exit after any IO is complete.\n");
  pangolin::QuitAll();
}
...
VideoViewer::~VideoViewer() { QuitAndWait(); }
...
void VideoViewer::Run()
{
  ...
  pangolin::CreateWindowAndBind(
      window_name, (int)(video.Width() * video.Streams().size()),
  ...
  for (size_t v = 0; v < container.NumChildren() && v < 9; v++) {
    ...
    pangolin::RegisterKeyPressCallback(
        screenshot_keys[v], [this, v, &images]() {
  ...
  while (should_run && !pangolin::ShouldQuit()) {
...

    {
      ...
      if (frame < grab_until &&
          video.Grab(&buffer[0], images, video_grab_wait, video_grab_newest)) {
        frame = frame + 1;

        if (frame_changed_callback) {
          frame_changed_callback(
              buffer.get(), images, GetVideoFrameProperties(video_interface));
        }
        ...
        if ((frame - 1) % draw_nth_frame == 0) {
          for (unsigned int i = 0; i < images.size(); ++i)
            if (stream_views[i].IsShown()) {
              stream_views[i].SetImage(
                  images[i],
                  pangolin::GlPixFormat(video.Streams()[i].PixFormat()));
            }
        }
      }
    }
...
void VideoViewer::RegisterDefaultKeyShortcutsAndPangoVariables()
{
  ...
  pangolin::RegisterKeyPressCallback(
      'd', [this]() { ToggleDiscardBufferedFrames(); });
...
void VideoViewer::OpenInput(std::string const& input_uri)
{
  ...
  video_playback = pangolin::FindFirstMatchingVideoInterface<
      pangolin::VideoPlaybackInterface>(video);
  video_interface =
      pangolin::FindFirstMatchingVideoInterface<pangolin::VideoInterface>(
          video);
...
{
  std::lock_guard<std::mutex> lock(control_mutex);
  grab_until = (current_frame < grab_until) ? current_frame
...
void VideoViewer::DrawEveryNFrames(int n)
{
  ...
  if (n != draw_nth_frame && n == 1) pango_print_info("Drawing every frame.\n");
...
bool VideoViewer::ChangeExposure(int delta_us)
{
  ...
  std::vector<pangolin::GenicamVideoInterface*> ifs =
      FindMatchingVideoInterfaces<pangolin::GenicamVideoInterface>(video);
  ...
  return ifs[active_cam]->SetParameter(
      "ExposureTime", std::to_string(exp + delta_us));
}
