...
IntensityImage<> LoadLibRaw(const std::string& filename)
{
  ...
  // TypedImage image(S.width, S.height, PixelFormatFromString("GRAY16LE"),
  // sizeof(uint16_t) * S.raw_width); PitchedCopy((char*)image.ptr, image.pitch,
  // (char*)RawProcessor.imgdata.rawdata.raw_image, sizeof(uint16_t) *
  // S.raw_width, sizeof(uint16_t) * image.w, image.h);
