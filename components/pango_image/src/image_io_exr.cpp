...
{
  public:
  StdIStream(std::istream& is) : Imf::IStream("stream"), _is(&is) {}

  virtual bool read(char c[/*n*/], int n)
  {
    if (!*_is) throw std::runtime_error("Unexpected end of file.");
    _is->read(c, n);
    if (_is->gcount() < n) {
  ...
  virtual Imf::Int64 tellg() { return std::streamoff(_is->tellg()); }

  virtual void seekg(Imf::Int64 pos) { _is->seekg(pos); }

  virtual void clear() { _is->clear(); }
...
PixelFormat GetPixelFormat(const Imf::Header& header)
{
      ...
      switch (channel.type) {
        ...
        default:
          throw std::invalid_argument(
              "Currently, only floating-point OpenEXR files are supported.");
      }
    }
    count += 1;
    pixelFormat << i.name();
  ...
  if (count != 1 && count != 3 && count != 4) {
    throw std::invalid_argument(
        "Currently, only 1, 3 or 4-channel OpenEXR files are supported.");
  }
...
IntensityImage<> LoadExr(std::istream& source)
{
  ...
  char* imgBase = (char*)img.ptr - (dw.min.x + dw.min.y * width) *
  ...
  auto writeChannel = [&](Imf::ChannelList::ConstIterator& i, size_t j) {
    fb.insert(
        i.name(),
        Imf::Slice(
            pixeltype, imgBase + pixelsize * j, pixelsize * format.channels,
    ...
    bool top_line_first)
{
  ...
  if (top_line_first) {
    ...
    for (size_t y = 0; y < image_in.h; ++y) {
      std::memcpy(
          flip_image.RowPtr(y), image_in.RowPtr(image_in.h - 1 - y),
          image_in.pitch);
    }
    image = flip_image;
  }
  ...
  for (unsigned int i = 0; i < fmt.channels; i++) {
    ...
    frameBuffer.insert(
        CHANNEL_NAMES[i], Imf::Slice(
                              channel->type, (char*)image.ptr + (ch_bits / 8),
                              fmt.bpp / 8,  // xstride
                              image.pitch   // ystride
                              ));
