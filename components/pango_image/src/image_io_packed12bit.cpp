...
void SavePacked12bit(const IntensityImage& image, std::ostream& out)
{
  ...
  const size_t dest_pitch = (image.width()*12)/ 8 + ((image.width()*12) % 8 > 0? 1 : 0);
  ...
  const char* fmt_str = ToString(image.pixelType());
  memcpy(header.magic, "P12B", 4);
  memset(header.fmt, '\0', sizeof(header.fmt));
  memcpy(header.fmt, fmt_str, std::min(sizeof(header.fmt), strlen(fmt_str)) );
...
IntensityImage LoadPacked12bit(std::istream& in)
{
  ...
  const size_t input_pitch = (img.width()*12)/ 8 + ((img.width()*12) % 8 > 0? 1 : 0);
