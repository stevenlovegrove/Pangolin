...
namespace std {
template <> struct hash<pangolin::LogKey> {
...
struct LoggingImpl : public Log
{
    ~LoggingImpl()
    {
        if(print_summary_on_close_ && num_ignored_.size() > 0) {
            ...
            for(auto [kind, count] : num_ignored_) {
                const fmt::color level_color = sLogKindColorTable[static_cast<int>(kind)];
                fmt::print("{}x {}   ", fmt::format(fg(level_color), "{}", kind), count);
            }
            println("");
        }
    }
    ...
    void logImpl(Kind level, const char *sFile, const char* sFunction, const int nLine, const char* assertion_statement, const std::string& description) override
    {
        ...
        // the maps during insertion.
        std::lock_guard<std::mutex> lk(log_mutex_);

        const auto last_err = last_error_for_line_.find({sFile,nLine});
        const bool already_printed = unique_only_ && last_err != last_error_for_line_.end() &&
