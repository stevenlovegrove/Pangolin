...
void PrintSchemeHelp(std::ostream& out, bool color)
{
  ...
  out << c_bold << "Using Factory Schemes" << c_normal << std::endl
      << std::endl;

  out << "The factory to use is specified in the 'scheme' portion of the URL. "
         "Any parameters the factory takes will be reset to default values."
      << std::endl;
  out << " e.g. \"" << c_scheme << "scheme" << c_normal << "://" << c_normal
      << "\"" << std::endl;
  out << std::endl;

  out << "When a factory needs to choose a unique resource, it can be "
         "specified using the URI portion of the URL."
      << std::endl;
  out << " e.g. \"" << c_scheme << "scheme" << c_normal << "://" << c_param
      << "uri" << c_normal << "\"" << std::endl;
  out << std::endl;

  out << "Parameters can be specified within square brackets to override "
         "defaults."
      << std::endl;
  out << " e.g. \"" << c_scheme << "scheme" << c_normal << ":[";
  out << c_param << "param1" << c_normal << "=" << c_alias << "value1"
      << c_normal << ",";
  out << c_param << "param2" << c_normal << "=" << c_alias << "value2"
      << c_normal << ",...";
    ...
    HelpVerbosity level, bool color)
{
  ...
  if (level >= HelpVerbosity::SYNOPSIS) {
    out << "Factory schemes can be accessed through any of their named aliases "
           "(shown below). If multiple factories support the same scheme, the "
           "factory having the lowest scheme precedence (shown in brackets) "
           "will be used."
        << std::endl;
  }
  if (level >= HelpVerbosity::PARAMS) {
    out << "Available parameters and their default values for the factories "
           "are also shown. Regular expressions may be used if the parameter "
           "itself can vary (see the examples)."
        << std::endl;
  }
  ...
  for (size_t idx : order) {
    if (scheme_filter.empty() ||
        factories[idx]->Schemes().count(scheme_filter) > 0) {
      PrintFactoryDetails(
          out, high_pri_names[idx] + post, *factories[idx], level,
          longest_scheme, color);
    }
  }
