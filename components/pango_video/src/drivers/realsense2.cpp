...
RealSense2Video::RealSense2Video(ImageDim dim, int fps) : dim_(dim), fps_(fps)
{
  sizeBytes = 0;
  ...
  {  // config color
    ...
    StreamInfo streamRGB(
        ...
        reinterpret_cast<uint8_t*>(sizeBytes));
...
void RealSense2Video::Stop() { pipe->stop(); }

size_t RealSense2Video::SizeBytes() const { return sizeBytes; }

std::vector<StreamInfo> const& RealSense2Video::Streams() const
{
  return streams;
}
...
size_t RealSense2Video::GetTotalFrames() const { return total_frames; }
...
PANGOLIN_REGISTER_FACTORY(RealSense2Video)
{
  ...
  return FactoryRegistry::I()->RegisterFactory<VideoInterface>(
      std::make_shared<RealSense2VideoFactory>());
}
