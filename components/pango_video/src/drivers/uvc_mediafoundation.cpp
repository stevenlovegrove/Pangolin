...
{
  public:
  AsyncSourceReader(IMFMediaSource* mediaSource, uint64_t timeout_ms) :
      ref_(1), timeout_(std::chrono::milliseconds(timeout_ms))
  {
    HRESULT hr;
    ...
    if (FAILED(hr)) {
      throw VideoException(FormatString(
          "Unable to initialize attributes for source reader (%)", hr));
    }

    hr = pAttributes->SetUnknown(MF_SOURCE_READER_ASYNC_CALLBACK, this);
    if (FAILED(hr)) {
      throw VideoException(FormatString(
          "Unable to set async attribute for source reader (%)", hr));
    }

    hr =
        MFCreateSourceReaderFromMediaSource(mediaSource, pAttributes, &reader_);
    if (FAILED(hr)) {
      throw VideoException(FormatString(
          "Unable to create source reader from UVC media source (%)", hr));
    }
  }
  ...
  HRESULT ReadSample(
      DWORD dwStreamIndex, DWORD dwControlFlags, DWORD* pdwActualStreamIndex,
      DWORD* pdwStreamFlags, LONGLONG* pllTimestamp, IMFSample** ppSample)
  {
    ...
    // If there's already a pending read, just piggyback on it and wait.
    // Otherwise, issue now.
    if (sample_hr_ != E_PENDING) {
      HRESULT sync_hr = reader_->ReadSample(
          dwStreamIndex, dwControlFlags, NULL, NULL, NULL, NULL);
      if (FAILED(sync_hr)) {
        return sync_hr;
      }
    ...
    while (sample_hr_ == E_PENDING) {
      if (sample_ready_.wait_until(lock, wait_target) ==
          std::cv_status::timeout) {
        // Leave sample_hr_ as E_PENDING since there's still an oustanding async
        // call. It may be set later by the callback.
...
bool UvcMediaFoundationVideo::GrabNext(unsigned char* image, bool wait)
{
  ...
  if (asyncSourceReader != nullptr) {
    hr = asyncSourceReader->ReadSample(
        (DWORD)MF_SOURCE_READER_FIRST_VIDEO_STREAM, 0, &streamIndex, &flags,
        &timeStamp, &sample);
  } else {
  ...
  if (SUCCEEDED(hr)) {
    ...
    std::this_thread::sleep_for(
        std::chrono::microseconds(1000000 / expected_fps));
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    frame_properties[PANGO_HOST_RECEPTION_TIME_US] =
        picojson::value(pangolin::Time_us(pangolin::TimeNow()));
  }
...
int UvcMediaFoundationVideo::IoCtrl(
    ...
    UvcRequestCode req_code)
{
  ...
  if (req_code == UVC_GET_CUR) {
    ...
    if (len != sizeof(uint16_t)) {
      std::cerr
          << "UVC_GET_LEN requested with a buffer not the size of uint16_t"
          << std::endl;
      return -1;
    }
    ...
    hr = ksControl->KsProperty(
        (PKSPROPERTY)&s, sizeof(s), NULL, 0, &ulBytesReturned);
  ...
  hr = ksControl->KsProperty(
      (PKSPROPERTY)&s, sizeof(s), data, len, &ulBytesReturned);

  if (hr == HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER)) {
    std::cerr << "IoCtrl result is " << ulBytesReturned
              << ", buffer supplied is " << len << ", result truncated."
              << std::endl;
    hr = S_OK;
  }
...
bool UvcMediaFoundationVideo::SetExposure(int exp_us)
{
  ...
  HRESULT hr = camera_control->Set(
      CameraControl_Exposure, from_100micros(exp_us / 100),
...
bool UvcMediaFoundationVideo::GetGain(float& gain)
{
  ...
  // (FW bug) We can't rely on gainCamMin/gainCamMax here, as they don't
  // correspond to the values returned by this API (HAL bug) This assumes that
  // the caller knows to normalize back into API space
...
bool UvcMediaFoundationVideo::SetGain(float gain)
{
  ...
  // (FW bug) We can't rely on gainCamMin/gainCamMax here, as they don't
  // correspond to the values supported by this API (HAL bug) This assumes that
  // the caller knows to normalize into camera register space [16, 256]
  long targetGain = gain;

  HRESULT hr = video_control->Set(
      VideoProcAmp_Gain, targetGain, VideoProcAmp_Flags_Manual);
  if (FAILED(hr)) {
    ...
    int vendorId, int productId, const std::string& instanceId)
{
  ...
  if (SUCCEEDED(hr)) {
    hr = CoCreateInstance(
        CLSID_SystemDeviceEnum, 0, CLSCTX_INPROC, IID_ICreateDevEnum,
        (void**)&dshowDevices);
  }

  IEnumMoniker* videoInputEnumerator = nullptr;
  if (SUCCEEDED(hr)) {
    hr = dshowDevices->CreateClassEnumerator(
        CLSID_VideoInputDeviceCategory, &videoInputEnumerator, 0);
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    while ((hr = videoInputEnumerator->Next(1, &moniker, 0)) == S_OK) {
      ...
      if (SUCCEEDED(hr) && variantPath.vt == VT_BSTR) {
        // Determine if this is the correct device by comparing its path against
        // the source's symbolic link This breaks the rules, but it seems to be
        // the only way to make sure it's the correct device

        // DirectShow and MediaFoundation appear to each create their own
        // symbolic link which contains a GUID Ignore the GUID portion of the
        // link, leaving just the path information
        size_t braceOffset = symLink.find(L'{');

        pathMatches =
            0 ==
            std::wcsncmp(symLink.c_str(), variantPath.bstrVal, braceOffset);
      } else {
        pathMatches = false;
      }
    ...
    if (SUCCEEDED(hr)) {
      hr = baseFilter->QueryInterface(
          __uuidof(IKsTopologyInfo), (void**)&ksTopologyInfo);
    }
    ...
    for (DWORD nodeIndex = 0; nodeIndex < numberOfNodes; ++nodeIndex) {
      ...
      if (nodeGuid == KSNODETYPE_DEV_SPECIFIC) {
        ...
        hr = ksTopologyInfo->CreateNodeInstance(
            nodeIndex, __uuidof(IUnknown), (void**)&pUnknown);

        if (SUCCEEDED(hr) && pUnknown != nullptr) {
          hr = pUnknown->QueryInterface(
              __uuidof(IKsControl), (void**)&ksControl);
        }
        if (SUCCEEDED(hr)) {
...
const GUID MFVideoFormat_Y10 = {0x20303159, 0x0000, 0x0010, 0x80, 0x00, 0x00,
                                0xAA,       0x00,   0x38,   0x9b, 0x71};

void UvcMediaFoundationVideo::InitDevice(
    size_t width, size_t height, bool async)
{
  HRESULT hr;
  ...
  for (DWORD i = 0;; ++i) {
    ...
    hr = sourceReader->GetNativeMediaType(
        MF_SOURCE_READER_FIRST_VIDEO_STREAM, i, &checkMediaType);
    if (FAILED(hr)) {
    ...
    if (FAILED(MFGetAttributeSize(
            checkMediaType, MF_MT_FRAME_SIZE, &checkWidth, &checkHeight))) {
      checkWidth = 0;
      checkHeight = 0;
    }

    int checkError =
        abs(int(checkWidth) - int(width)) + abs(int(checkHeight) - int(height));
  ...
  if (bestMediaType) {
    if (SUCCEEDED(hr)) {
      sourceReader->SetCurrentMediaType(
          MF_SOURCE_READER_FIRST_VIDEO_STREAM, nullptr, bestMediaType);
      width = bestWidth;
      height = bestHeight;
      if (hasValidStride) {
        stride = bestStride;
      ...
      if (bestGuid == MFVideoFormat_YUY2) {
        ...
        bit_depth = 10;
          ...
          (unsigned char)((bestGuid.Data1 >> 0) & 0xff) == 'Y' &&
          (unsigned char)((bestGuid.Data1 >> 8) & 0xff) == '8') {
        pixelFormat = PixelFormatFromString("GRAY8");
        bit_depth = 8;
      } else {
        pango_print_warn(
            "Unexpected MFVideoFormat with FOURCC %c.%c.%c.%c",
  ...
  if (SUCCEEDED(hr)) {
    hr = mediaSource->QueryInterface(
        __uuidof(IAMCameraControl), (void**)&camera_control);
    if (FAILED(hr)) {
      // On failure, soldier on without manual exposure controls
      pango_print_warn(
          "Failed to populate IAMCameraControl (0x%x). Exposure control will "
          "not function",
          hr);
    }
  }
  ...
  const StreamInfo stream_info(
      pixelFormat, width, height, size_bytes / height, 0);
...
void UvcMediaFoundationVideo::PopulateGainControls()
{
  ...
  HRESULT hr = mediaSource->QueryInterface(
      __uuidof(IAMVideoProcAmp), (void**)&video_control);

  // read the min and max values from the camera to allow mapping from our fixed
  // ranges
  if (SUCCEEDED(hr)) {
    long steppingDelta;
    long flags;

    // FW bug: this range is being reported as [0, 1024], but the Get/Set
    // pathway handles register values [16, 256]
    hr = video_control->GetRange(
        VideoProcAmp_Gain, &gainCamMin, &gainCamMax, &steppingDelta,
        &gainCamDefault, &flags);

    // should we be checking that flags are correctly set to
...
{
  struct UvcMediaFoundationVideoFactory final
    ...
    ParamSet Params() const override
    {
      return {{
          ...
          {"period", "0",
           "Specify frame period in microseconds (0:unspecified)"},
    ...
    std::unique_ptr<VideoInterface> Open(const Uri& uri) override
    {
      ...
      std::istringstream(uri.url.substr(uri.url.find("vid_", 0) + 4, 4)) >>
          std::hex >> vendorId;
      std::istringstream(uri.url.substr(uri.url.find("pid_", 0) + 4, 4)) >>
          std::hex >> productId;
      ...
      return std::unique_ptr<VideoInterface>(new UvcMediaFoundationVideo(
          vendorId, productId, instanceId, dim.x, dim.y, fps));
    }
  };

  return FactoryRegistry::I()->RegisterFactory<VideoInterface>(
      std::make_shared<UvcMediaFoundationVideoFactory>());
}
}  // namespace pangolin
