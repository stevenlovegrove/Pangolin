...
MjpegVideo::MjpegVideo(std::string const& filename)
{
  ...
  if (file_type != ImageFileType::ImageFileTypeJpg) {
    throw VideoException(
        full_path +
        " has no jpeg header when attempting to open as mjpeg stream.");
  }
  ...
  streams.emplace_back(
      next_image.fmt, next_image.w, next_image.h, next_image.pitch, nullptr);
...
size_t MjpegVideo::GetCurrentFrameId() const { return next_frame_id - 1; }

size_t MjpegVideo::GetTotalFrames() const { return offsets.size(); }
...
{
  struct MjpegVideoFactory final
  ...
  return FactoryRegistry::I()->RegisterFactory<VideoInterface>(
      std::make_shared<MjpegVideoFactory>());
}
