...
RealSenseVideo::RealSenseVideo(ImageDim dim, int fps) : dim_(dim), fps_(fps)
{
  ctx_ = new rs::context();
  sizeBytes = 0;
  for (int32_t i = 0; i < ctx_->get_device_count(); ++i) {
    ...
    devs_[i]->enable_stream(
        rs::stream::depth, dim_.x, dim_.y, rs::format::z16, fps_);
    StreamInfo streamD(
        PixelFormatFromString("GRAY16LE"), dim_.x, dim_.y, dim_.x * 2, 0);
    streams.push_back(streamD);

    sizeBytes += streamD.SizeBytes();
    devs_[i]->enable_stream(
        rs::stream::color, dim_.x, dim_.y, rs::format::rgb8, fps_);
    StreamInfo streamRGB(
        PixelFormatFromString("RGB24"), dim_.x, dim_.y, dim_.x * 3,
        (uint8_t*)0 + sizeBytes);
...
RealSenseVideo::~RealSenseVideo() { delete ctx_; }
...
bool RealSenseVideo::GrabNext(unsigned char* image, bool wait)
{
  ...
  for (int32_t i = 0; i < ctx_->get_device_count(); ++i) {
    ...
    memcpy(
        out_img, devs_[i]->get_frame_data(rs::stream::depth),
        streams[i * 2].SizeBytes());
    out_img += streams[i * 2].SizeBytes();
    memcpy(
        out_img, devs_[i]->get_frame_data(rs::stream::color),
        streams[i * 2 + 1].SizeBytes());
...
size_t RealSenseVideo::GetCurrentFrameId() const { return current_frame_index; }

size_t RealSenseVideo::GetTotalFrames() const { return total_frames; }
...
PANGOLIN_REGISTER_FACTORY(RealSenseVideo)
{
  ...
  return FactoryRegistry::I()->RegisterFactory<VideoInterface>(
      std::make_shared<RealSenseVideoFactory>());
}
