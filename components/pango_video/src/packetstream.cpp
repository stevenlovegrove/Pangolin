...
size_t PacketStream::readUINT()
{
  ...
  if (!good()) return static_cast<size_t>(-1);
...
PangoTagType PacketStream::readTag(PangoTagType x)
{
  auto r = readTag();
  if (r != x)
    throw std::runtime_error(("Tag mismatch error: expected tag '" +
                              tagName(r) + "' does not match found tag '" +
                              tagName(x) + "'")
                                 .c_str());
  return r;
}
...
PangoTagType PacketStream::syncToTag()  // scan through chars one by one until
                                        // the last three look like a tag
{
  peekTag();
