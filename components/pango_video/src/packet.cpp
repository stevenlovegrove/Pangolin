...
Packet::Packet(
    PacketStream& s, std::unique_lock<std::recursive_mutex>&& lock,
...
Packet::Packet(Packet&& o) :
    ...
    frame_streampos(o.frame_streampos),
    _stream(o._stream),
    lock(std::move(o.lock)),
    data_streampos(o.data_streampos),
    _data_len(o._data_len)
{
  o._data_len = 0;
}

Packet::~Packet() { ReadRemaining(); }

size_t Packet::BytesRead() const { return _stream.tellg() - data_streampos; }
    ...
    PacketStream& s, std::vector<PacketStreamSource>& srcs)
{
  ...
  src = s.readUINT();
  PANGO_ENSURE(
      json_src == size_t(-1) || json_src == src,
      "Frame preceded by metadata for a mismatched source. Stream may be "
      "corrupt.");
