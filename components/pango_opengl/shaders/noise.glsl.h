...
float perlin_noise(in vec2 p)
{
  ...
  vec2 u =
      f * f *
      (3.0 - 2.0 * f);  // feel free to replace by a quintic smoothstep instead

  return mix(
      mix(dot(grid_noise(i + ivec2(0, 0)), f - vec2(0.0, 0.0)),
          dot(grid_noise(i + ivec2(1, 0)), f - vec2(1.0, 0.0)), u.x),
      mix(dot(grid_noise(i + ivec2(0, 1)), f - vec2(0.0, 1.0)),
          dot(grid_noise(i + ivec2(1, 1)), f - vec2(1.0, 1.0)), u.x),
      u.y);
}
