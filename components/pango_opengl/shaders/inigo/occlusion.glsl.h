...
float ftriOcclusion(
    in vec3 pos, in vec3 nor, in vec3 v0, in vec3 v1, in vec3 v2)
{
...
float fquadOcclusion(
    in vec3 pos, in vec3 nor, in vec3 v0, in vec3 v1, in vec3 v2, in vec3 v3)
{
  vec3 a = normalize(v0 - pos);
  vec3 b = normalize(v1 - pos);
  vec3 c = normalize(v2 - pos);
  ...
  return (dot(nor, normalize(cross(a, b))) * acos(dot(a, b)) +
          ...
          dot(nor, normalize(cross(d, a))) * acos(dot(d, a))) /
         6.283185;
}
