...
#define PANGO_VAR_TYPES(x)                                                     \
  x(bool) x(int8_t) x(uint8_t) x(int16_t) x(uint16_t) x(int32_t) x(uint32_t)   \
      x(int64_t) x(uint64_t) x(float) x(double)
...
#define PANGO_CHECK_WRAP(x)                                                    \
  if (!strcmp(v->TypeId(), typeid(x).name())) {                                \
    std::shared_ptr<VarValueT<x>> xval =                                       \
        std::dynamic_pointer_cast<VarValueT<x>>(v);                            \
