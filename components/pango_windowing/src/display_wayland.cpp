...
static const std::map<enum xdg_toplevel_resize_edge, std::string>
    resize_cursor = {
...
struct ButtonSurface {
  ...
  enum type { CLOSE = 100, MAXIMISE } function;

  ButtonSurface(
      wl_compositor *compositor, wl_subcompositor *subcompositor,
      wl_surface *source, EGLDisplay egl_display, EGLConfig config, int32_t x,
      int32_t y, uint width, uint height, type fnct, pangolin::Color colour) :
      ...
      function(fnct)
  {
    ...
    subsurface =
        wl_subcompositor_get_subsurface(subcompositor, surface, source);
    egl_context =
        eglCreateContext(egl_display, config, EGL_NO_CONTEXT, nullptr);
    egl_window = wl_egl_window_create(surface, width, height);
    egl_surface = eglCreateWindowSurface(
        egl_display, config, (EGLNativeWindowType)egl_window, nullptr);
  }
      ...
      const int main_w, const int main_h, int &x, int &y, int &w, int &h) const
  {
    ...
    switch (function) {
      ...
      case XDG_TOPLEVEL_RESIZE_EDGE_LEFT:
        ...
        y = -title_bar_size;
        w = border_size;
        h = main_h + title_bar_size;
        break;
      case XDG_TOPLEVEL_RESIZE_EDGE_TOP_LEFT:
        x = -border_size;
        y = -border_size - title_bar_size;
        w = border_size;
        h = border_size;
        break;
...
struct Decoration {
  Decoration(
      const uint border_size, const uint title_size,
      const pangolin::Color colour, wl_compositor *compositor,
      wl_subcompositor *subcompositor, wl_surface *surface,
  ...
  void create()
  {
    ...
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_NONE, colour);

    // sides, 1D resizing
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_LEFT, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_RIGHT, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_TOP, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM, colour);

    // corners, 2D resizing
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_TOP_LEFT, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_TOP_RIGHT, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_LEFT, colour);
    decorations.emplace_back(
        compositor, subcompositor, surface, egl_display, config, border_size,
        title_size, XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_RIGHT, colour);

    // buttons
    buttons.reserve(2);
    buttons.emplace_back(
        compositor, subcompositor, decorations[0].surface, egl_display, config,
        5, 1, button_width, button_height, ButtonSurface::type::CLOSE,
        pangolin::Color(0, 110 / 255.0, 182 / 255.0));
    buttons.emplace_back(
        compositor, subcompositor, decorations[0].surface, egl_display, config,
        10 + button_width, 1, button_width, button_height,
        ButtonSurface::type::MAXIMISE, pangolin::Color(1.0, 204 / 255.0f, 0));
  }
  ...
  {
    return resize_cursor.count((enum xdg_toplevel_resize_edge)last_type)
               ? resize_cursor.at((enum xdg_toplevel_resize_edge)last_type)
               : "left_ptr";
  }
...
struct WaylandWindow : public WindowInterface {
  ...
  // we can only attach a buffer to a surface after it has been configured at
  // least once
...
static const std::map<uint, KeyModifier> wl_key_mod_ids = {
    {KEY_LEFTSHIFT, KeyModifierShift}, {KEY_RIGHTSHIFT, KeyModifierShift},
    {KEY_LEFTCTRL, KeyModifierCtrl},   {KEY_RIGHTCTRL, KeyModifierCtrl},
    {KEY_LEFTALT, KeyModifierAlt},     {KEY_RIGHTALT, KeyModifierAlt},
};

static const std::map<uint, int> wl_key_special_ids = {
    {KEY_F1, PANGO_KEY_F1},          {KEY_F2, PANGO_KEY_F2},
    {KEY_F3, PANGO_KEY_F3},          {KEY_F4, PANGO_KEY_F4},
    {KEY_F5, PANGO_KEY_F5},          {KEY_F6, PANGO_KEY_F6},
    {KEY_F7, PANGO_KEY_F7},          {KEY_F8, PANGO_KEY_F8},
    {KEY_F9, PANGO_KEY_F9},          {KEY_F10, PANGO_KEY_F10},
    {KEY_F11, PANGO_KEY_F11},        {KEY_F12, PANGO_KEY_F12},

    {KEY_LEFT, PANGO_KEY_LEFT},      {KEY_UP, PANGO_KEY_UP},
    {KEY_RIGHT, PANGO_KEY_RIGHT},    {KEY_DOWN, PANGO_KEY_DOWN},

    {KEY_PAGEUP, PANGO_KEY_PAGE_UP}, {KEY_PAGEDOWN, PANGO_KEY_PAGE_DOWN},
    {KEY_HOME, PANGO_KEY_HOME},      {KEY_END, PANGO_KEY_END},
    {KEY_INSERT, PANGO_KEY_INSERT},
};

static void handle_configure_toplevel(
    ...
    wl_fixed_t sx, wl_fixed_t sy)
{
  ...
  if (w->pressed) {
    w->MouseMotionSignal(
        MouseMotionEvent{{float(w->lastx), float(w->lasty), d->flags}});
  } else {
    w->PassiveMouseMotionSignal(
        MouseMotionEvent{{float(w->lastx), float(w->lasty), d->flags}});
  }
}
  ...
  if (w->decoration->last_type < 0) {
    ...
    if ((button == BTN_LEFT) && (state == WL_POINTER_BUTTON_STATE_PRESSED)) {
      switch (w->decoration->last_type) {
        ...
        case XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_RIGHT:
          xdg_toplevel_resize(
              w->xshell_toplevel, d->wseat, serial, w->decoration->last_type);
          break;
...
static const struct wl_pointer_listener pointer_listener = {
    ...
    pointer_handle_axis,
#if WAYLAND_VERSION_GE(1, 12)
    pointer_handle_frame,     pointer_handle_axis_source,
    ...
    uint32_t size)
{
  if ((!data) || (format != WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1)) {
    std::cerr << "wrong keymap format, got " << format
              << ", expected WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1" << std::endl;
    close(fd);
    return;
  }
  ...
  if (keymap_string == MAP_FAILED) {
    std::cerr << "keymap mmap failed: " << std::string(std::strerror(errno))
              << std::endl;
    close(fd);
    return;
  }
  xkb_keymap_unref(d->keymap);
  d->keymap = xkb_keymap_new_from_string(
      d->xkb_context, keymap_string, XKB_KEYMAP_FORMAT_TEXT_V1,
    ...
    uint32_t group)
{
  ...
  xkb_state_update_mask(
      d->xkb_state, mods_depressed, mods_latched, mods_locked, 0, 0, group);
}

#if WAYLAND_VERSION_GE(1, 12)

static void keyboard_handle_repeat_info(
    void * /*data*/, struct wl_keyboard * /*wl_keyboard*/, int32_t /*rate*/,
    int32_t /*delay*/)
{
}
...
static void seat_handle_capabilities(
    void *data, struct wl_seat *seat, uint32_t caps1)
{
    ...
    const char *interface, uint32_t version)
{
  ...
  if (strcmp(interface, wl_compositor_interface.name) == 0) {
    w->wcompositor = reinterpret_cast<wl_compositor *>(wl_registry_bind(
        registry, id, &wl_compositor_interface,
        std::min<uint32_t>(version, 4)));
  } else if (strcmp(interface, wl_subcompositor_interface.name) == 0) {
    w->wsubcompositor = static_cast<wl_subcompositor *>(wl_registry_bind(
        registry, id, &wl_subcompositor_interface,
        std::min<uint32_t>(version, 1)));
  } else if (strcmp(interface, xdg_wm_base_interface.name) == 0) {
    w->xshell = reinterpret_cast<xdg_wm_base *>(wl_registry_bind(
        registry, id, &xdg_wm_base_interface, std::min<uint32_t>(version, 3)));
  } else if (strcmp(interface, wl_seat_interface.name) == 0) {
    w->wseat = reinterpret_cast<wl_seat *>(wl_registry_bind(
        registry, id, &wl_seat_interface, std::min<uint32_t>(version, 5)));
    wl_seat_add_listener(w->wseat, &seat_listener, data);
  } else if (strcmp(interface, wl_shm_interface.name) == 0) {
    w->shm = static_cast<wl_shm *>(wl_registry_bind(
        registry, id, &wl_shm_interface, std::min<uint32_t>(version, 1)));
    ...
    display(display)
{
  ...
  egl_surface = eglCreateWindowSurface(
      display->egl_display, display->egl_configs[0],
  ...
  decoration = std::unique_ptr<Decoration>(new Decoration(
      5, 20, grey, display->wcompositor, display->wsubcompositor, wsurface,
      display->egl_display, display->egl_configs[0]));
  decoration->create();
...
{
  eglMakeCurrent(
      display->egl_display, egl_surface, egl_surface, display->egl_context);
}

void WaylandWindow::RemoveCurrent()
{
  eglMakeCurrent(
      display->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
}
...
PANGOLIN_REGISTER_FACTORY(WaylandWindow)
{
  ...
  return FactoryRegistry::I()->RegisterFactory<WindowInterface>(
      std::make_shared<WaylandWindowFactory>());
}
