...
TEST_CASE("Rethrow")
{
  ...
  REQUIRE_THROWS([]() { auto t = !makeSomething(73, false); }());
}
...
TEST_CASE("Conversions")
{
  ...
  CHECK_THROWS([]() { Shared<Base> x = std::unique_ptr<Derived>(nullptr); }());
  CHECK_THROWS([]() { Shared<Base> x = std::shared_ptr<Derived>(nullptr); }());
}
