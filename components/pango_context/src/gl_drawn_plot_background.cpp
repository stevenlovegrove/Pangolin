...
struct GlDrawnPlotBackground : public DrawnPlotBackground
{
    ...
    void draw( const ViewParams& params) override {
        ...
        Eigen::Matrix4d graph_from_clip = (params.clip_from_image * params.image_from_camera * params.camera_from_world).inverse();
        Eigen::Array2d graph_min = (graph_from_clip * Eigen::Vector4d(-1.0,-1.0, 0.0, 1.0)).head<2>();
        Eigen::Array2d graph_max = (graph_from_clip * Eigen::Vector4d(+1.0,+1.0, 0.0, 1.0)).head<2>();
        Eigen::Array2d graph_range = (graph_max - graph_min).abs();
        Eigen::Array2d graph_per_pix = graph_range / params.viewport.range().cast<double>();
        ...
        u_unit_graph_start = Eigen::Array2f(
            std::pow(t2t, log10_start.x()),
            std::pow(t2t, log10_start.y())
        );
...
private:
    const Shared<GlSlProgram> prog = GlSlProgram::Create({
        .sources = {{ .origin="/components/pango_opengl/shaders/main_plot_background.glsl" }}
    });
